...
class TestConfig:
    ...
    def test_valid_front_door(self):
        ...
        assert isinstance(p._adapter, FrontDoorAdapter)

    def test_valid_app_gateway(self):
        p = _make_provider(waf_type="app_gateway")
        assert isinstance(p._adapter, AppGatewayAdapter)
...
class TestProperties:
    ...
    def test_account_name_returns_resource_group(self):
        p = _make_provider()
        assert p.account_name == "rg-test"

    def test_zone_plans_empty_initially(self):
        p = _make_provider()
        assert p.zone_plans == {}
...
class TestConcurrentWorkers:
    ...
    def test_concurrent_partial_failure(self, _sleep):
        ...
        client = MagicMock()
        client.policies.get.return_value = MagicMock()
        p = _make_provider(client=client, max_workers=3)
        for name in policies:
            p.resolve_zone_id(name)

        def mock_get(rg, name):
            if name == "pb":
                raise HttpResponseError("server error", response=MagicMock(status_code=500))
        ...
        with ThreadPoolExecutor(max_workers=3) as executor:
            futures = {
                executor.submit(
                    p.get_phase_rules, Scope(zone_id=name, label=""), "azure_waf_custom"
                ): name
                for name in policies
            }
            results = {}
    ...
    def test_concurrent_auth_error_propagates(self):
        ...
        def mock_get(rg, name):
            if name == "pa":
                raise ClientAuthenticationError("forbidden")
            return {
                "custom_rules": {"rules": []},
            }
...
# ---------------------------------------------------------------------------
# Feature: account_name returns resource_group
# ---------------------------------------------------------------------------
class TestAccountNameReturnsResourceGroup:
    def test_returns_resource_group(self):
        p = _make_provider()
        assert p.account_name == "rg-test"
    ...
    def test_custom_resource_group(self):
        ...
        assert p.account_name == "my-custom-rg"

    @patch.dict(
        "os.environ",
        {"AZURE_SUBSCRIPTION_ID": "s", "AZURE_RESOURCE_GROUP": "env-rg"},
    )
    def test_env_var_resource_group(self):
        p = AzureWafProvider(waf_type="front_door", client=MagicMock())
        assert p.account_name == "env-rg"


# ---------------------------------------------------------------------------
# Feature: zone_plans populated with SKU tier
# ---------------------------------------------------------------------------
class TestZonePlans:
    def test_premium_front_door(self):
        client = MagicMock()
        p = _make_provider(client=client)
        client.policies.get.return_value = {
            "name": "my-policy",
    ...
    def test_standard_front_door(self):
        ...
        p.resolve_zone_id("my-policy")
        assert p.zone_plans == {"my-policy": "standard"}
    ...
    def test_classic_front_door(self):
        ...
        p.resolve_zone_id("my-policy")
        assert p.zone_plans == {"my-policy": "classic"}

    def test_waf_v2_app_gateway(self):
        client = MagicMock()
        p = _make_provider(client=client, waf_type="app_gateway")
        client.web_application_firewall_policies.get.return_value = {
            "name": "agw-policy",
            "sku": {"name": "WAF_v2"},
            "custom_rules": [],
        }
        p.resolve_zone_id("agw-policy")
        assert p.zone_plans == {"agw-policy": "waf_v2"}

    def test_unknown_sku_lowercased(self):
        client = MagicMock()
        p = _make_provider(client=client)
        client.policies.get.return_value = {
            "name": "my-policy",
            "sku": {"name": "FutureTier_V3"},
            "custom_rules": {"rules": []},
        }
        p.resolve_zone_id("my-policy")
        assert p.zone_plans == {"my-policy": "futuretier_v3"}
    ...
    def test_sku_name_is_empty(self):
        client = MagicMock()
        p = _make_provider(client=client)
        client.policies.get.return_value = {
            "name": "my-policy",
    ...
    def test_multiple_zones(self):
        ...
        client.policies.get.side_effect = mock_get
        p.resolve_zone_id("policy-a")
        p.resolve_zone_id("policy-b")
        assert p.zone_plans == {"policy-a": "premium", "policy-b": "standard"}


# ---------------------------------------------------------------------------
# Feature: get_all_phase_rules resilience with failed_phases
# ---------------------------------------------------------------------------
class TestGetAllPhaseRulesResilience:
    @patch("octorules.retry.time.sleep")
    def test_transient_error_populates_failed_phases(self, _sleep):
        from octorules.provider.base import Scope
        ...
        result = p.get_all_phase_rules(Scope(zone_id="my-policy"))
        assert dict(result) == {}
        assert sorted(result.failed_phases) == [
            "azure_waf_custom",
            "azure_waf_managed",
            "azure_waf_rate",
        ]
    ...
    @patch("octorules.retry.time.sleep")
    def test_transient_error_with_specific_phases(self, _sleep):
        from octorules.provider.base import Scope
        ...
        result = p.get_all_phase_rules(
            Scope(zone_id="my-policy"), provider_ids=["azure_waf_custom"]
        )
        assert dict(result) == {}
        assert result.failed_phases == ["azure_waf_custom"]

    def test_auth_error_still_propagates(self):
        from octorules.provider.base import Scope

        client = MagicMock()
        p = _make_provider(client=client)
        client.policies.get.side_effect = ClientAuthenticationError("forbidden")
        with pytest.raises(ProviderAuthError):
            p.get_all_phase_rules(Scope(zone_id="my-policy"))

    @patch("octorules.retry.time.sleep")
    def test_auth_classified_http_error_propagates(self, _sleep):
        from octorules.provider.base import Scope

        client = MagicMock()
        p = _make_provider(client=client)
        err = HttpResponseError("authorization failed")
        err.error = MagicMock()
        err.error.code = "AuthorizationFailed"
        client.policies.get.side_effect = err
        with pytest.raises(ProviderAuthError):
    ...
    @patch("octorules.retry.time.sleep")
    def test_connection_error_populates_failed_phases(self, _sleep):
        from octorules.provider.base import Scope
    ...
    def test_success_still_returns_empty_failed_phases(self):
        from octorules.provider.base import Scope
...
# ---------------------------------------------------------------------------
# _normalize_sku unit tests
# ---------------------------------------------------------------------------
class TestNormalizeSku:
    def test_all_known_skus(self):
        from octorules_azure.provider import _normalize_sku

        assert _normalize_sku({"sku": {"name": "Premium_AzureFrontDoor"}}) == "premium"
        assert _normalize_sku({"sku": {"name": "Standard_AzureFrontDoor"}}) == "standard"
        assert _normalize_sku({"sku": {"name": "Classic_AzureFrontDoor"}}) == "classic"
        assert _normalize_sku({"sku": {"name": "WAF_v2"}}) == "waf_v2"

    def test_unknown_sku_lowercased(self):
        from octorules_azure.provider import _normalize_sku

        assert _normalize_sku({"sku": {"name": "NewTier_Preview"}}) == "newtier_preview"

    def test_no_sku_key(self):
        from octorules_azure.provider import _normalize_sku

        assert _normalize_sku({}) is None

    def test_sku_not_dict(self):
        from octorules_azure.provider import _normalize_sku

        assert _normalize_sku({"sku": "not-a-dict"}) is None

    def test_sku_name_none(self):
        from octorules_azure.provider import _normalize_sku

        assert _normalize_sku({"sku": {"name": None}}) is None

    def test_sku_name_empty(self):
        from octorules_azure.provider import _normalize_sku

        assert _normalize_sku({"sku": {"name": ""}}) is None
